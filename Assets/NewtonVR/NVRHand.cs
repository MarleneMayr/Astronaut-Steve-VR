...
namespace NewtonVR
{
    public class NVRHand : MonoBehaviour
    {
        ...
        protected virtual void Update()
        {
            ...
            //UpdateColliders(); //own function from Marlene
        }

        protected void UpdateColliders()
        {
            //Trigger for Index Finger is not Touched
            if (HoldButtonPressed && !Inputs[UseButton].IsTouched)
            {
                if (IsLeft)
                {
                    Debug.Log("Grip pressed and Trigger not touched. - Left");
                    CustomPhysicalColliders = Player.HandLeft;
                }
                else if (IsRight)
                {
                    Debug.Log("Grip pressed and Trigger not touched. - Right");
                    CustomPhysicalColliders = Player.HandRight;
                }

                Player.UpdateHand(this);
                //InputDevice.Initialize(this);
                //InitializeRenderModel();
                PhysicalController.SetupCustomModel();
            }

            //Trigger for Index Finger is Pressed
            if (UseButtonPressed)
            {
                if (IsLeft)
                {
                    Debug.Log("Trigger pressed. - Left");
                    CustomPhysicalColliders = Player.HandLeftFist;
                }
                else if (IsRight)
                {
                    Debug.Log("Trigger pressed. - Left");
                    CustomPhysicalColliders = Player.HandRightFist;
                }
